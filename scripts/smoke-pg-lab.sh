...
if ! docker run --rm --network "${NET}" \
  ...
  go test -p 1 ./internal/storage/... ./adapters/driven/persistence/postgres/... \
    -run 'TestOpenPostgres_LiveSchema|TestIngestSequencer_PgLab|TestSrtHealthRepository_PgLab|TestProjectTokenRepository_PgLab|TestEventRepository_PgLab|TestIngestStreamRepository_PgLab' \
    -v -count=1; then
