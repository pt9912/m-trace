...
§Lifecycle als State Machine — der Zustand ist das Verzeichnis, kein
Status-Feld.
...
- Eine **Welle** (Bündel von Slices) ebenso: Der Zustand ist die
  Verzeichnis-Position, kein `Status:`-Feld. Der Welle-Plan (`welle-<NN>-*.md`)
...
- Der aktive Durchlauf `open/` → `next/` → `in-progress/` nimmt ausschließlich
  ...
  **Nicht-Slice-Records** — Welle-Plan, Welle-Closure
  (`done/welle-<NN>-results.md`) und der grandfatherte
  `plan-<version>.md`-Bestand ([MR-007](../../../harness/conventions.md#mr-007)).
  Aufgelöste Carveouts wandern **nicht** hierher, sondern in ihr eigenes
  `docs/plan/carveouts/done/` (Baseline-Regelwerk `modul-07-carveouts.md`).

Neben den Lifecycle-Verzeichnissen liegt **flach** in `planning/` das
**Beobachtungs-Register** ([`observations/`](observations/)): Es trägt den
...
m-trace hat **kein** `reconciliation.md` — das Register für offene
Brownfield-Bootstrap-Funde (Baseline-Regelwerk `modul-02-harness-bootstrap.md`
§Das Reconciliation-Register) ist für Repos gedacht, die aus einem
dokumentierten Rückbau-Vorgang kommen; m-trace hat keinen solchen
durchlaufen.
...
Verzeichnissen (`open/`, `next/`, `in-progress/`, `done/`).
