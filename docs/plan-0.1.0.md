...
- [ ] Zwei neue MVP-Endpoints aus Lastenheft §7.3:
    ...
    - `GET /api/stream-sessions/{id}` (Detail mit Event-Liste). Event-Liste mit Default-Limit 100, hartes Maximum 1000 (Query-Parameter `events_limit`); stabile Sortierung nach `(server_received_at asc, sequence_number asc, event_id asc)` — `event_id` (oder Hash über `client_timestamp`+`event_name`+SDK-Felder, falls keine ID existiert) als finaler Tie-Breaker. Cursor-basierte Pagination via Query-Parameter `events_cursor`. Hintergrund: Cardinality/Storage-Risiko aus Lastenheft §7.10 — der Endpoint darf nicht unbeschränkt viele Events streamen.
- [ ] **CORS / Origin-Validierung** für Browser-SDK-Anbindung (Voraussetzung für `0.1.1`). Architektur-Variante **B** (User-Entscheidung): Preflight prüft Origin gegen die globale Union aller Allowed-Origins; der echte `POST` validiert zusätzlich `Origin` ↔ `project_id`/Token-Bindung. Strengere Preflight-Isolation pro Project (Variante A) ist als Folge-ADR vorgesehen, siehe Roadmap §4.
    ...
    - Preflight-Verhalten (kein Body, keine Auth, keine Project-Auflösung):
        - **Player-SDK-Pfad** `OPTIONS /api/playback-events` — Bezug **NF-33 + NF-35 + NF-36**: Origin in globaler Union aller Allowed-Origins → `204 No Content` mit `Access-Control-Allow-Origin: <origin>` (konkret, niemals `*`), `Access-Control-Allow-Methods: POST, OPTIONS` (NF-35), `Access-Control-Allow-Headers: Content-Type, X-MTrace-Project, X-MTrace-Token` (NF-36), `Access-Control-Allow-Credentials: false`, `Access-Control-Max-Age ≈ 600`. Origin nicht in der Union → `403 Forbidden`.
