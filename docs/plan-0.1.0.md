...
- [ ] **`ingest_sequence` als serverseitiges Pflichtfeld** im Domain-Modell `PlaybackEvent`: monoton steigender Counter pro `apps/api`-Prozess (oder pro Persistenz-Adapter, wenn auf SQLite migriert wird), gesetzt im Use Case `RegisterPlaybackEventBatch` direkt vor `Append`. Zusätzlich zu `client_timestamp`, `server_received_at`, `sequence_number` (alle aus dem Wire-Format). Begründet die Eindeutigkeit der Pagination-Sortierung auch bei identischen Client-Feldern.
- [ ] **Cursor-Invalidierung nach Storage-Restart** (Pflicht für In-Memory-Default): Cursor kapselt eine `process_instance_id` (z. B. UUID, beim Setup einmalig erzeugt) zusätzlich zu den Sortier-Feldern. Beim Inbound-Request wird die `process_instance_id` aus dem Cursor mit der aktuellen Prozess-`process_instance_id` verglichen — Mismatch → `400 Bad Request` mit Body `{"error":"cursor_invalid","reason":"storage_restart"}`. Der Client muss vom Anfang neu paginieren. **Single-Instance-Annahme**: gilt explizit nur für `0.1.0` mit Single-`apps/api`-Container und in-memory `EventRepository`. Bei späterer Horizontalskalierung oder Blue/Green-Deployments würde dieselbe Cursor-Logik bei jedem Restart und bei Cross-Instance-Routing invalidieren — eine durabel-konsistente Cursor-Strategie ist Folge-ADR-Thema (Roadmap §4). SQLite-Migration (OE-3-Folge-ADR) wird die Persistenz fortführen; bis dahin ist Restart = Cursor-Invalidierung. Tests: (a) Cursor ohne Restart funktioniert, (b) Cursor nach Server-Restart liefert `400 cursor_invalid`.
- [ ] **CORS / Origin-Validierung** für Browser-SDK-Anbindung (Voraussetzung für `0.1.1`). Architektur-Variante **B** (User-Entscheidung): Preflight prüft Origin gegen die globale Union aller Allowed-Origins; der echte `POST` validiert zusätzlich `Origin` ↔ `project_id`/Token-Bindung. Strengere Preflight-Isolation pro Project (Variante A) ist als Folge-ADR vorgesehen, siehe Roadmap §4.
...
- [ ] **Rate-Limit-Dimensionen erweitert (F-110)**: `RateLimiter`-Port erweitert oder neu modelliert für drei Dimensionen `project_id` / `origin` / `client_ip`. Spike-Implementierung deckt nur `project_id` ab; für `0.1.0` werden mindestens zwei Dimensionen (`project_id`, `client_ip`) als Token-Buckets implementiert. `origin` als dritter Bucket ist Pflicht für Browser-Traffic im `0.1.1`-Test, kann aber bereits in `0.1.0` mit-implementiert werden. Konfiguration über Konstanten oder ENV-Variablen analog Spike-Pattern. **Architektur-Sync (Pflicht-Begleit-DoD)**: `docs/architecture.md` §3.3 (Driven-Ports-Code-Beispiel) muss synchron auf die neue Port-Signatur (`Allow(ctx, projectID, origin, clientIP string, n int)` oder strukturierte Eingabe) gehoben werden; API-Kontrakt §6 (Rate Limiting) ergänzt die drei Dimensionen explizit. Ohne diesen Sync läuft das Zielbild gegenüber dem Plan auseinander.
- [ ] **F-22** (Lastenheft `1.1.3` §7.3, Wording aus Patch `1.1.0`): Architektur-Vorbereitung — Port `hexagon/port/driven/StreamAnalyzer` mit konkretem (kleinem) no-op-fähigem Vertrag, **kein** Marker-Interface (in Go würden leere Interfaces von jedem Typ erfüllt — nicht hilfreich als Port-Vertrag). Konkrete Soll-Form:
    ```go
    type StreamAnalyzer interface {
        AnalyzeBatch(ctx context.Context, events []domain.PlaybackEvent) error
    }
    ```
    Adapter `adapters/driven/streamanalyzer/NoopStreamAnalyzer`: Implementierung gibt `nil` zurück, mit `var _ driven.StreamAnalyzer = (*NoopStreamAnalyzer)(nil)`-Compile-Time-Check. Use Case verdrahtet den Port im Konstruktor (Slot ist gesetzt, falls eine Folge-Phase ihn füttern will), ruft die Methode aber nicht produktiv auf. Damit ist die Architektur-Erweiterungs-Stelle real (mit Compile-Time-Garantie), ohne Produktiv-Last in `0.1.0`. Volle Integration ab Phase `0.3.0`.
