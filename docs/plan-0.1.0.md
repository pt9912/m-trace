...
- [ ] **CORS / Origin-Validierung** für Browser-SDK-Anbindung (Voraussetzung für `0.1.1`):
    ...
    - **NF-33 + NF-35 + NF-36** Preflight-fähige CORS-Konfiguration im HTTP-Adapter, deckt zwei Endpoint-Klassen ab:
        - **Player-SDK-Pfad** (`POST /api/playback-events`): `OPTIONS` antwortet mit `Access-Control-Allow-Methods: POST, OPTIONS`, `Access-Control-Allow-Headers: Content-Type, X-MTrace-Project, X-MTrace-Token`, konkretem `Access-Control-Allow-Origin` (kein `*`, sobald Project-Tokens genutzt werden — NF-34), `Access-Control-Max-Age` ≈ 600 s.
        - **Dashboard-Lese-Pfad** (`GET /api/stream-sessions`, `GET /api/stream-sessions/{id}`, `GET /api/health`): `OPTIONS` antwortet mit `Access-Control-Allow-Methods: GET, OPTIONS` und denselben Headers/Origin-Regeln; `Access-Control-Allow-Credentials: false`. Voraussetzung für Dashboard-Browser-Zugriffe in `0.1.1`, sobald Dashboard und API über getrennte Origins erreichbar sind (Compose-Lab-Setup; siehe Hinweis in `plan-0.1.1.md` §3 zur SvelteKit-Dev-Proxy als Alternative im Vite-Dev-Mode).
