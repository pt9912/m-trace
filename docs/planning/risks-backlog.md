...
| R-5 | Time-Skew-Persistenz auf Event-Ebene fehlt: `0.4.0` setzt `mtrace.time.skew_warning=true` als Span-Attribut (siehe `spec/telemetry-model.md` §2.5/§5.3), aber die Schema-Spalte und Dashboard-Anzeige sind explizit deferred. Folge: skew-betroffene Events sind im Read-Pfad (Dashboard ohne Tempo) nicht sichtbar markiert; Operator muss in Tempo schauen. | `plan-0.4.0.md` §3.1 | `0.5.0` oder Folge-Tranche bei Bedarf | ⬜ | Schema-Spalte (`time_skew_warning BOOLEAN`) ergänzen + EventRepository füllt sie + Dashboard-Timeline blendet sie ein. Auslösender Bedarf: erste reale Skew-Beobachtung im Lab. |
