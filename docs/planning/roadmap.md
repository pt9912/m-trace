...
| Durabel-konsistente Cursor-Strategie für Pagination           | `0.4.0`, parallel zur SQLite-Migration                         | `0.1.0` nutzt `process_instance_id` im Cursor; Restart bzw. Cross-Instance-Routing invalidiert den Cursor. Persistenz-Folge-ADR (ADR-0002) hat die Storage-Garantien geklärt (SQLite); Folge-ADR muss eine durabel-stabile Cursor-Form (z. B. opaker Token mit Storage-Token-ID, durable Sequence-Generator) festlegen.                                                                                |
