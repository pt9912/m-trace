...
| R-25 | Die Readback-Reconciliation in `smoke-load.sh` (Gate „kein stiller Verlust": `persisted >= accepted`) zählte die persistierten Events, indem sie den Detail-Endpoint **seitenweise zu je 1000 Events über HTTP paginierte** — über alle Sessions. Das war **O(N) über das Gesamt-Eventvolumen**: Beim 4h-Dispatch-Soak (`27628293077`, 45,68 Mio akzeptierte Events / 20 Sessions) waren das ~45,7k sequentielle Reads → **~2h reine Readback-Zeit ohne jede Log-Ausgabe** (Command-Substitution `persisted="$(…)"`, druckte erst am Ende). Zusammen mit den 4h Last sprengte der „Run load smoke"-Step den **GitHub-6h-Job-Cap** → Job gecancelt, **Verdict-Step skipped, kein Load-Readiness-Verdict** (ADR-0005-Trigger #3 ungemessen). Für kurze Smokes (≤ einige 100k Events) unkritisch; nur der lange Soak-Pfad war betroffen — der Nightly-SLO-Smoke (`27661025846`, open-loop kurz) blieb grün. | `scripts/smoke-load.sh` (Readback-Block); `.github/workflows/load-smoke.yml` (6h-Job-Cap); Run `27628293077` (cancelled, 6h); `docs/planning/done/plan-0.22.5-load-smoke.md` §5 Soak-Dispatch-Log | Tranche 4 ✅ | 🟢 | **gelöst** — Fix-Option (a) geliefert in `scripts/smoke-load.sh`: Im Autostart-Pfad (`SMOKE_LOAD_AUTOSTART=1`, der CI-/Soak-Pfad) werden die persistierten Events per **direktem `SELECT count(*) FROM playback_events WHERE session_id GLOB '<prefix>-*'`** gegen das api-SQLite-Volume gezählt — O(1)-Read statt O(N)-HTTP-Pagination. Eine eigene `sqlite3`-Instanz (`SQLITE_IMAGE`, Default `keinos/sqlite3`, `--entrypoint sqlite3 --user 0:0 --volumes-from api`) liest dieselbe WAL-DB (`SQLITE_DB_PATH`, Default `/var/lib/mtrace/m-trace.db`) mit; das ist derselbe Persistenz-Beleg, aus dem auch der Detail-Endpoint serviert, nur direkt — deckt sich mit der Skript-Absicht „die TATSÄCHLICH in `playback_events` liegenden Events". GLOB statt LIKE (behandelt `_` literal). Der portable HTTP-Readback bleibt **Fallback für `SMOKE_LOAD_AUTOSTART=0`** (ad-hoc gegen fremdes Lab). Lesefehler → INCONCLUSIVE (Exit 3, inkl. sqlite-stderr in der Diagnose), nie Verlust-FAIL. **Verifiziert** lokal via kurzem `make smoke-soak` (DURATION=10s/8 VUs): COUNT instant, `persisted 9900 == accepted 9900`, Retention-Probe erstmals erreicht (vorher nie). **Real-Scale bestätigt**: der re-dispatchte 4h-Soak `27665523746` zählte **55.327.560 Events instant** (`persisted == accepted`) und lief in **4h02m grün** durch (vs. 6h-Cancel davor) — Tranche-4-Verdict erreicht (s. `plan-0.22.5-load-smoke.md` §5 Soak-Dispatch-Log). |
| R-26 | **Multi-Tenant-/High-Traffic-Lastnachweis fehlt.** Der Load-/Soak-Smoke treibt Last mit **genau einem Projekt/Token** (`demo-token`, im Lab hardcoded in `apps/api/cmd/api/main.go` `projectConfigs`). Belegt ist damit Single-Instance-Lab-Lastfähigkeit unter kontrollierter Parallelität (NF-20/22/23, s. `plan-0.22.5-load-smoke.md` §7) — **nicht** belegt: produktive **Multi-Tenant**-Isolation (N Projekte gleichzeitig, Per-Projekt-Rate-Limit-Fairness, Noisy-Neighbor) und erst recht nicht **Multi-Replica**-Skalierung (≥ 2 api-Instanzen). | `scripts/load/playback-events.k6.js` (Single-Token); `apps/api/cmd/api/main.go` (`projectConfigs`, ein Projekt); Per-Projekt-Ingest-Limiter (`MTRACE_RATE_LIMIT_CAPACITY`, projekt-skopiert, existiert); `docs/perf/budgets.md` §7; ADR-0005 Trigger #1 | Load-Smoke-Folgetranche | ⬜ | **Machbarkeit (geprüft 2026-06-17)**: (a) **High-Traffic single-instance** ist **belegt** über das Open-Loop-Profil (`LOAD_PROFILE=open`, `TARGET_EVENT_RATE`): Nachweis-Lauf `27696206008` (open-loop **2000 ev/s**, 10 min, 1,2 Mio Events) grün — **p95 7,6ms** ≪ 1s-Budget, 0 `rate_limited`, 0 Fehler, 0 dropped, `persisted == accepted`. Zusammen mit dem 4h-Soak (3842 ev/s closed-loop @ p95 636ms) hält die Instanz also weit jenseits der Nightly-Baseline mit großem Headroom. Die echte **Sättigungsdecke wurde noch nicht erreicht** (beide Läufe graceful); ein Saturations-Lauf *jenseits* der Decke bräuchte zusätzlich `VUS`/`P95_BUDGET_MS` als `workflow_dispatch`-Inputs (heute nur `mode`/`duration`/`target_event_rate`) — kleine Workflow-Erweiterung. (b) **Multi-Tenant** ist mit kleiner Tooling-Erweiterung machbar, kein Architektur-Blocker: env-getriebenes Seeding von N Lab-Projekten (**additiv** zum Default-`demo`-Projekt, byte-stabiler Default), k6-Variante mit Token-Fan-out über N Tokens, eigener `make`-Mode; misst Per-Projekt-Rate-Limit-Isolation, Fairness, Noisy-Neighbor. **Achtung (Review 2026-06-17)**: der Per-Projekt-Ingest-Limiter ist **in-process** (`ratelimit.NewTokenBucketRateLimiter`, keine Redis-Variante — Redis nur für Origin/Issuance, R-22/R-17); gegen *eine* Instanz testbar, aber über N Replicas ist die effektive Decke `N × Capacity` und Fairness nicht repliken-übergreifend. Echter Multi-Tenant-Fairness-Nachweis *unter Scale-out* setzt daher zusätzlich einen **shared (Redis) Ingest-Limiter** voraus (Teil dieser R-26-b-Arbeit, **nicht** von plan-0.23.0, das nur den Store/R-26 c löst). (c) **Multi-Replica** ist auf der SQLite-Single-Instance-Architektur **nicht** beweisbar — setzt den per ADR-0005 **Trigger #1** (≥ 2 Replicas → Postgres) bewusst zurückgestellten Postgres-Adapter voraus. **Jetzt adressiert (2026-06-17)**: `ADR-0006` reaktiviert RAK-91 („defer" → „proceed, optional"), Ausführung tranchenweise in `plan-0.23.0-postgres-scaleout` (`docs/planning/open/`) — liefert den Scale-out-Lasttest, der diese R-26-c-Lücke mit Messwerten schließt. **Eröffnungs-Trigger**: konkreter Multi-Tenant-Produktions-Readiness-Bedarf, oder als nächste Load-Smoke-Tranche nach `plan-0.22.5-load-smoke.md`. |
| R-27 | **Postgres-Adapter: Keyset-Pagination-Skip/Dup unter Concurrent-Writern (Read-Side).** Die Cursor-Pagination ordnet **primär über `server_received_at ASC`**, dann `sequence_number`, `ingest_sequence` nur als finaler Tie-Breaker (`event_repository.go:68`: `ORDER BY server_received_at, COALESCE(sequence_number,…), ingest_sequence`; ADR-0004). `server_received_at` wird im App-Layer zur Empfangszeit gesetzt (`ServerReceivedAt: now`), **nicht** zur Commit-Zeit. Sobald es nebenläufige Postgres-Writer gibt (= Adapter existiert, Tranche 2), kann ein Writer ein Event mit `server_received_at = T-1` *nach* einem Reader committen, der schon an `T` vorbeipaginiert ist → **übersprungen**, auch bei perfekt monotonem `ingest_sequence`. **Unabhängig von R-28** (nicht ingest_sequence-zentrisch). | `apps/api/adapters/driven/persistence/postgres` (neu, plan-0.23.0); `apps/api/adapters/driven/persistence/sqlite/event_repository.go` (Cursor-`ORDER BY`); ADR-0004; ADR-0006; `plan-0.23.0-postgres-scaleout.md` §3/Tranche 3 | plan-0.23.0 Tranche 3 (vor Scale-out-Test) | ⬜ | **Belegpflicht/Mitigation**: Concurrent-Insert-Test, der explizit **out-of-order `server_received_at`-Commit** abdeckt (spät committeter Writer mit früherem `server_received_at` darf nicht übersprungen werden), nicht nur Sequenz-Kollision; Cursor-Walk = jedes Event genau einmal. **`REPEATABLE READ` allein reicht nicht** (Pagination = mehrere Queries über mehrere Snapshots); cross-page trägt nur ein **commit-order-stabiles Wasserzeichen** (nicht über noch-nicht-sichtbare Commits hinauspaginieren). **DoD-blockierend für Tranche 3.** **Eröffnungs-Trigger**: jetzt — DoD-Punkt von plan-0.23.0; operativ nach R-28 testbar (sonst maskieren PK-Kollisionen alles), aber **keine** kausale Abhängigkeit. |
| R-28 | **Postgres-Adapter: der `ingest_sequencer` ist ein In-Process-RAM-Counter — Write-Side-Multi-Replica-Blocker.** `NewIngestSequencer` seedet einen `atomic.Int64` einmalig aus `SELECT MAX(ingest_sequence)`, `Next()` = `counter.Add(1)`; der Adapter inserted den app-zugewiesenen Wert explizit in `ingest_sequence INTEGER PRIMARY KEY` (`AUTOINCREMENT` nur Defense-in-Depth). Zwei API-Replicas seeden aus demselben MAX und vergeben **dieselben** Werte → garantierte Primary-Key-Kollisionen / doppelte `ingest_sequence` genau im Multi-Replica-Betrieb (dem Plan-Zweck). Heute nur unsichtbar, weil SQLite Single-Instance ist. | `apps/api/adapters/driven/persistence/sqlite/ingest_sequencer.go`; `apps/api/hexagon/port/driven/ingest_sequencer.go` (`driven.IngestSequencer`-Vertrag); `apps/api/adapters/driven/persistence/postgres` (neu); ADR-0006; `plan-0.23.0-postgres-scaleout.md` §3/Tranche 2 | plan-0.23.0 Tranche 2 (Voraussetzung für Multi-Replica, vor R-27) | ⬜ | **Belegpflicht/Mitigation**: Der Postgres-Sequencer muss **DB-autoritativ** werden. **`SELECT nextval` ist der port-erhaltende Weg** (bevorzugt): synchron, `int64`, hält `driven.IngestSequencer.Next() int64` 1:1 → SQLite-/InMemory-Impl + Call-Site (`register_playback_event_batch.go`, Pre-Assign) unangetastet; nur das `SELECT MAX`-Seeding entfällt. **`IDENTITY` + `RETURNING` vermeiden**: Wert erst nach dem INSERT bekannt → bricht den Pre-Assign-Flow (Use-Case-/Domain-/Drei-Adapter-Refactor). **Perf**: per-Event-`nextval` ist ein Roundtrip (heute `atomic.Add`, 0 I/O) → Block-Allokation pro Batch hinter dem Port (Sequence-`CACHE n`/`generate_series`), sonst konfundiert es das Tranche-6-Scale-out-Verdict. **Implementierungs-Redesign, kein Dialekt-Spiegel** — widerlegt ADR-0006-„kein Rewrite" für diesen Port. **Eröffnungs-Trigger**: jetzt — ohne dies ist Multi-Replica gar nicht lauffähig (operativ vor R-27, das aber unabhängig ist). |
