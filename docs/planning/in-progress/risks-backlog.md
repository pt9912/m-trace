...
| R-27 | **Postgres-Adapter: `ingest_sequence`-Monotonie + Cursor-Ordering unter Concurrent-Writern.** SQLite serialisiert per Single-Writer (`_txlock=immediate`) → `ingest_sequence` (AUTOINCREMENT) ist trivial monoton. Der Postgres-Adapter (plan-0.23.0) erlaubt echte Parallel-Writer (sein Sinn) — eine IDENTITY/Sequence ist eindeutig, aber **Commit-Reihenfolge ≠ Sequence-Reihenfolge** (Lücken, out-of-order-Sichtbarkeit). Die Keyset-Cursor-Pagination ordnet über `(server_received_at, sequence_number, ingest_sequence)` (`event_repository.go:68`, ADR-0004); unter Concurrent-Insert ist zu belegen, dass kein Event übersprungen oder doppelt geliefert wird. **Read-Side**-Risiko; setzt das **R-28**-Redesign (DB-autoritativer Sequencer) voraus — der heutige In-Process-Counter nutzt gar keine DB-`IDENTITY`, daher wird R-27 erst *nach* R-28 real. DoD-Punkt von plan-0.23.0. | `apps/api/adapters/driven/persistence/postgres` (neu, plan-0.23.0); `apps/api/adapters/driven/persistence/sqlite/event_repository.go` (Cursor-Tupel); ADR-0004; ADR-0006; `plan-0.23.0-postgres-scaleout.md` §3/Tranche 3 | plan-0.23.0 Tranche 2/3 (vor Scale-out-Test) | ⬜ | **Belegpflicht/Mitigation**: dedizierter Concurrent-Insert-Test (N Writer parallel) + Cursor-Walk, der jedes Event genau einmal sieht — **DoD-blockierend für Tranche 3**, vor Tranche 6 (Scale-out) zu schließen. Lösungsoptionen z. B. Snapshot-/`REPEATABLE READ`-Isolation für den Read-Pfad oder eine commit-order-stabile Sequenzquelle. **Eröffnungs-Trigger**: jetzt — nicht konditional, weil es ein DoD-Punkt von plan-0.23.0 ist. |
| R-28 | **Postgres-Adapter: der `ingest_sequencer` ist ein In-Process-RAM-Counter — Write-Side-Multi-Replica-Blocker.** `NewIngestSequencer` seedet einen `atomic.Int64` einmalig aus `SELECT MAX(ingest_sequence)`, `Next()` = `counter.Add(1)`; der Adapter inserted den app-zugewiesenen Wert explizit in `ingest_sequence INTEGER PRIMARY KEY` (`AUTOINCREMENT` nur Defense-in-Depth). Zwei API-Replicas seeden aus demselben MAX und vergeben **dieselben** Werte → garantierte Primary-Key-Kollisionen / doppelte `ingest_sequence` genau im Multi-Replica-Betrieb (dem Plan-Zweck). Heute nur unsichtbar, weil SQLite Single-Instance ist. | `apps/api/adapters/driven/persistence/sqlite/ingest_sequencer.go`; `apps/api/hexagon/port/driven/ingest_sequencer.go` (`driven.IngestSequencer`-Vertrag); `apps/api/adapters/driven/persistence/postgres` (neu); ADR-0006; `plan-0.23.0-postgres-scaleout.md` §3/Tranche 2 | plan-0.23.0 Tranche 2 (Voraussetzung für Multi-Replica, vor R-27) | ⬜ | **Belegpflicht/Mitigation**: Der Postgres-Sequencer muss **DB-autoritativ** werden — `nextval` einer Postgres-Sequence bzw. `GENERATED ... AS IDENTITY` + `RETURNING` statt `MAX`-Seed + `atomic.Add`. Ändert (1) das `SELECT MAX`-Seeding (weg/anders), (2) das „expliziter Wert im INSERT"-Muster (→ `RETURNING` bei IDENTITY), (3) ggf. den `driven.IngestSequencer`-Port-Vertrag (SQLite-/InMemory-Impl müssen grün bleiben). **Port-Redesign, kein Dialekt-Spiegel** — widerlegt das ADR-0006-„kein Rewrite" für genau diesen Port. **Eröffnungs-Trigger**: jetzt — ohne dies ist Multi-Replica gar nicht lauffähig. Schließt R-27 auf: sobald DB-autoritativ, wird der Wert zur Commit-Zeit vergeben → R-27s Read-Side-Ordering wird real. |
