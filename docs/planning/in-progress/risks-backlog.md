...
| R-22 | Origin-/IP-nahes Rate-Limiting fehlte strukturell. `0.12.0`/`0.12.5`-Limiter waren beide **Project-Token-skopiert** — IP-basierte Bursts (Botnet, Replay aus vielen Subnets) konnten den Project-Layer umgehen. | `docs/planning/done/plan-0.12.0.md` §10 OS-6; `apps/api/hexagon/port/driven/origin_rate_limiter.go`; `apps/api/adapters/driven/auth/in_memory_origin_rate_limiter.go`; `apps/api/adapters/driving/http/origin_rate_limit.go`; `docs/user/auth.md` §5.9 | `0.12.6` Tranche 6 (Memory) ✅; Redis bleibt Resttrigger für Tranche 7 | ⬜ | **teilweise gelöst** — In-Memory-Layer geliefert in `0.12.6` Tranche 6: neuer Driven-Port `driven.OriginRateLimiter` mit `Allow(ctx, key)`; `InMemoryOriginRateLimiter` (Single-Bucket pro Key, Capacity 20 / Refill 5/s; opportunistisches Idle-Eviction); HTTP-Middleware `originRateLimitMiddleware` vor `POST /api/auth/session-tokens` und `POST /api/playback-events`; Reihenfolge `Origin-Limit → Project-Limit`; `MTRACE_ORIGIN_RATE_LIMITER=disabled|memory` mit Boot-Validator-Rejects für `sqlite` (Multi-Host-unsicher) und `redis`/`memcached` (Folge-Item T7 mit R-17, Backend-Konsistenz); `MTRACE_TRUST_FORWARDED_FOR`-Opt-in für Reverse-Proxy mit Spoofing-Warning in der Operator-Doku. Reject-Body `{"error":"origin_rate_limited"}` (429). Tests decken Adapter (Burst, Refill, Key-Isolation, Disabled, Empty-Key, Context-Cancel) und HTTP-Roundtrip (Burst, XFF-Trust mit unabhängigen Buckets, Disabled-Pfad); `make smoke-origin-rate-limit` als Operator-Smoke. **Resttrigger** (für T7-Bundle mit R-17): Multi-Host-IP-Limits brauchen Network-Backend (Redis); ohne Network-Backend misst der Limiter pro Replica. **Wieder-Eröffnungs-Trigger nach T7**: Operator-Report über Cross-Replica-IP-Spoofing-Surface oder Bedarf an Memcached-Backend (gemeinsam mit Issuance-Limiter aus §9). |
