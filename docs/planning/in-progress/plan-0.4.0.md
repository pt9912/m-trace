...
- **Storage-/Repository-Prerequisite:** Der aktuelle `stream_sessions`-Store ist vor Tranche 3 `session_id`-global (`primary_key: [session_id]`, Repository-Reads per `Get(ctx, session_id)`). Tranche 3 muss Schema, Ports, Repository-Methoden, Resolver, InMemory-Adapter und SQLite-Adapter auf projekt-skopierte Session-Lookups heben: eindeutiger Key `(project_id, session_id)` und alle `Get`/`Upsert`-/Resolver-Pfade nehmen `project_id` entgegen. Eine Beweisoption über global eindeutige client-gelieferte `session_id`s ist nicht zulässig. Cross-Project-Kollisionstest ist Pflicht: gleiche `session_id` in zwei Projekten darf keine gemeinsame `correlation_id` erzeugen.
...
- **Analyzer-Linkage-Priorität:** Für explizite Analyzer-Verknüpfung gilt: `correlation_id` gewinnt vor `session_id`, aber nur innerhalb eines gültig aufgelösten Project-Kontexts. Verlinkte `POST /api/analyze`-Requests müssen `X-MTrace-Token` (und falls später aktiv: `X-MTrace-Project`) erfolgreich auf ein `project_id` auflösen; ohne gültigen Project-Kontext bleibt die Analyse `session_link.status="detached"` und darf nicht über `correlation_id`/`session_id` linken. Alle Link-Lookups laufen über `(project_id, correlation_id)` bzw. `(project_id, session_id)`. Wenn beide IDs angegeben sind, muss `session_id` im selben Project zur Session mit dieser `correlation_id` auflösen; bei Mismatch antwortet die API weiter mit `200 OK` in der neuen Hülle `{ "analysis": <AnalysisResult>, "session_link": { "status": "conflict_detached" } }`; das Ergebnis wird nicht in die Player-Timeline gemischt. Nur `session_id` ist als Fallback für bestehende oder bereits selbst-geheilte Sessions erlaubt; eine unbekannte `session_id` erzeugt durch `POST /api/analyze` keine neue Session und führt zu `session_link.status="not_found_detached"`.
- **Degradationsfälle:** Fehlende oder blockierte Resource-Timing-Daten, CORS-Lücken, Service-Worker-Interception, CDN-Redirects und hls.js-Signale mit unvollständigen Details erzeugen ein Netzwerkevent mit `meta.network.detail_status="network_detail_unavailable"` und optionalem `meta.network.unavailable_reason` gemäß `spec/telemetry-model.md` §1.4. Wenn ein Browser-/Native-HLS-Pfad gar kein Manifest-/Segment-Signal liefert, wird kein synthetisches Netzwerkereignis erfunden; stattdessen persistiert/API-exponiert Tranche 3 eine Korrelationsgrenze `network_signal_absent` außerhalb des Event-Streams. Die Dashboard-Sichtbarkeit dieser Grenze ist Tranche-4-Scope. Beide Varianten sind akzeptierte Degradation und müssen getrennt getestet werden.
- **Schema-Entscheidung:** Tranche 3 verwendet ausschließlich additive Felder im bestehenden Event-Wire-Schema `1.0` (z. B. neue `event_name`-Werte plus `meta.network.kind`/Timing-Felder). Kein Breaking Change und keine neue Major-Schema-Version in `0.4.0`; falls ein benötigtes Feld nicht additiv modellierbar ist, wird es deferred statt per Migration erzwungen. Neue `event_name`-Werte müssen in Legacy-Pipelines entweder als no-op akzeptiert oder als unbekannter, aber angenommener Event-Typ gezählt werden; sie dürfen keinen 4xx auslösen, solange die Pflichtfelder valide sind.
- **URL-Datenschutz:** Segment-/Manifest-URLs dürfen weder Prometheus-Labels noch rohe, credential-haltige Persistenzwerte werden. Vor Persistenz/Anzeige gilt die feste Redaction-Matrix: Scheme, Host und nicht-sensitive Pfadsegmente dürfen erhalten bleiben; Query und Fragment werden vollständig entfernt; `userinfo` wird entfernt; signierte/credential-artige Query-Parameter (`token`, `signature`, `sig`, `expires`, `key`, `policy` und case-insensitive Varianten) werden nicht gespeichert; ein Pfadsegment ist tokenartig, wenn es ≥ 24 Zeichen lang ist und überwiegend Base64URL-/Hex-Zeichen enthält oder bekannte JWT-/SAS-/Signed-URL-Muster trägt. Tokenartige Pfadsegmente werden ausschließlich durch `:redacted` ersetzt; es wird kein stabiler Hash und kein Gleichheitsmarker persistiert. Tests decken Query-/Fragment-Redaction, `userinfo`, signierte Query-Parameter, Token-Parameter und JWT-/Base64URL-Pfadsegmente ab.
...
- [ ] Storage-/Repository-Prerequisite ist implementiert: Schema/Migration stellt projekt-skopierte Session-Eindeutigkeit her, Port-Signaturen und Application-Resolver nehmen `project_id`, InMemory- und SQLite-Adapter nutzen `(project_id, session_id)` für `Get`/`Upsert`/Self-Healing.
- [ ] Cross-Project-Kollisionstest deckt InMemory und SQLite ab: dieselbe `session_id` in zwei Projekten erzeugt getrennte Sessions und unterschiedliche `correlation_id`s; Analyzer-Linking kann nicht projektübergreifend auflösen.
...
- [ ] Event-Schema bleibt `1.0` und unterscheidet Manifest-Request, Segment-Request und Player-Zustandsereignis additiv über Event-Namen und/oder `meta`-Felder. Contract-Tests sichern, dass ältere Backends unbekannte additive Felder weiter ignorieren und neue Backends die neuen Typen lesen; zusätzlich deckt ein konkret benanntes `0.3.x`-Backend-Fixture neue `event_name`-Werte und unbekannte `meta.network.*`-Felder als akzeptierten no-op bzw. unbekannten Event-Typ ohne 4xx ab.
...
- [ ] URL-Redaction ist vor Persistenz und Dashboard-Anzeige umgesetzt und getestet: keine Query-/Fragment-Speicherung, kein `userinfo`, keine signierten Query-Parameter, keine Credential-/Token-Parameter, keine tokenartigen Rohpfadanteile, keine URL-Labels in Prometheus. Der Event-Store enthält nur redigierte URL-Repräsentanten gemäß Abnahmegrenze und keinen stabilen Token-Hash.
...
- [ ] Tests decken die Degradationsmatrix ab: fehlende SDK-Felder, blockierte Browser-Timings, CORS-/Resource-Timing-Lücken als `network_detail_unavailable`-Events sowie mindestens ein Native-/Browser-Limitierungsfall ohne Netzwerksignal als API-/Persistenzgrenze `network_signal_absent` werden als akzeptierte Degradation geprüft; Dashboard-Anzeige folgt in Tranche 4.
- [ ] Analyzer-Ergebnisse aus `POST /api/analyze` werden ohne explizite Session-/`correlation_id`-Bindung oder ohne gültigen Project-Kontext nicht in die Player-Timeline gemischt; mit expliziter Bindung ist die Verknüpfung über `(project_id, correlation_id)` testbar. Tests decken die Response-Hülle `{analysis, session_link}` für nur `correlation_id`, nur vorhandenen `session_id`-Fallback, unbekannte `session_id` (`not_found_detached`), beide konsistent, beide im Konflikt (`conflict_detached`), fehlenden/ungültigen Project-Kontext (`detached`) und Cross-Project-Mismatch ab. Alle Fälle sind dokumentiert.
...
- [ ] Detailansicht zeigt `network_signal_absent`-Korrelationsgrenzen aus Tranche 3 als sichtbaren, nicht-fehlerhaften Hinweis, ohne synthetische Manifest-/Segment-Events zu erfinden.
