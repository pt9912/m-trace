...
- [ ] Detailansicht zeigt `network_signal_absent`-Korrelationsgrenzen aus Tranche 3 als sichtbaren, nicht-fehlerhaften Hinweis, ohne synthetische Manifest-/Segment-Events zu erfinden; `spec/backend-api-contract.md` definiert dafür im Session-Block `network_signal_absent[]` mit Feldnamen, Typen, leerem Default, Sortierung und Dedupe-Regel.
- [ ] Laufende Sessions sind von beendeten Sessions unterscheidbar; `session_ended` und Sweeper-Ende werden sichtbar. Für Sweeper-only Session-Änderungen ist explizit entschieden: entweder persistiert die API eine durable `session_update_sequence`/ein Lifecycle-Event, das SSE-Backfill abdecken kann, oder Sweeper-Sichtbarkeit ist bewusst Polling-only und wird mit einem eigenen Fallback-Test abgesichert.
...
- [ ] SSE-CORS ist für neue Stream-Routen explizit verdrahtet: `OPTIONS /api/stream-sessions/stream` und optional `OPTIONS /api/stream-sessions/{id}/events/stream` erlauben bei bekanntem Origin `GET, OPTIONS` und `Access-Control-Allow-Headers: X-MTrace-Token, X-MTrace-Project`. Tests decken erlaubten Origin, unbekannten Origin, fehlenden Token nach erfolgreichem Preflight und Header-Forwarding im fetch-basierten SSE-Client ab.
- [ ] SSE-`id`/`Last-Event-ID` ist an ein dauerhaft persistiertes Feld gebunden. Playback-Event-Backfill darf `ingest_sequence` nutzen; Session-Lifecycle-Updates ohne Playback-Event, insbesondere Sweeper-Ende/Stalled-Übergänge, brauchen entweder eine eigene durable `session_update_sequence`/Lifecycle-Event-ID oder sind ausdrücklich vom SSE-Backfill ausgenommen und werden nur über Polling sichtbar. Scope und Eindeutigkeit sind passend zum Stream-Typ definiert (globaler Stream braucht global eindeutige ID, Session-Stream mindestens session-eindeutige ID); Reconnect-Backfill liest ausschließlich aus SQLite und funktioniert nach API-Restart.
