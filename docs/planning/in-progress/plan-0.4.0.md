...
Bezug: RAK-32; MVP-14; F-38; F-39/F-40 nur gemäß Abnahmegrenzen unten; ADR 0002.
...
- [ ] F-39 (`API-Status anzeigen`) bleibt in Tranche 4 auf Session-/API-Erreichbarkeit und vorhandene Read-Fehlerzustände begrenzt; aggregierte Invalid-/Dropped-/Rate-Limit-Zähler werden erst mit Tranche 6 sichtbar, sofern kein vorheriger API-Status-Summary-Vertrag eingeführt wird.
- [ ] F-40 (`Links zu Grafana, Prometheus und Media-Server-Konsole`) wird in Tranche 4 entweder als reine konfigurationsgetriebene Link-Section ohne Live-Status umgesetzt und dokumentiert, oder explizit nach Tranche 6/Observability verschoben. Es gibt keine stillschweigende F-40-Erfüllung ohne DoD-Test.
...
- [ ] SSE-Endpunkt-Schnittstelle ist im `spec/backend-api-contract.md` als verlässlicher Vertrag dokumentiert: globaler Stream, optionaler Session-Detail-Stream, Payload-Schema, `Last-Event-ID`-/Backfill-Regel, Fehler-/Reconnect-Semantik und Polling-Fallback-Intervalle. Mindestpayloads: `event_appended` enthält `project_id`, `session_id`, `ingest_sequence`, `event_name` und optional `event_count`; `session_updated` enthält `project_id`, `session_id`, `state`, `last_seen_at`, `ended_at`, `event_count` und `correlation_id`. Wenn kein Detailstream gebaut wird, muss der globale Stream diese Mindestpayloads liefern, damit Detailansichten live aktualisieren können; ein reiner Listen-Invalidierungsstream erfüllt die Tranche-4-SSE-DoD nicht.
