...
- [ ] HTTP-Read-Handler lösen Project-Kontext vor Session-List, Session-Detail, Event-Read und Analyzer-Linking auf und reichen `project_id` in die Application-/Repository-Pfade weiter; Cursor enthalten oder validieren den Project-Scope, sodass Cursor aus Project A nicht für Project B nutzbar sind.
...
- [ ] Event-Schema bleibt `1.0` und nutzt für Netzwerkereignisse die bestehenden Event-Namen `manifest_loaded`/`segment_loaded` plus additive `meta.network.*`-Felder. `contracts/event-schema.json`, `packages/player-sdk/src/types/events.ts` und Public-API-Snapshot bleiben bezüglich Event-Name-Union unverändert; Contract-Tests sichern, dass ältere Backends unbekannte additive `meta.network.*`-Felder weiter ignorieren und neue Backends die neuen Meta-Felder lesen.
