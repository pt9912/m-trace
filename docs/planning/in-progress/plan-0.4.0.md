...
> **Status**: 🟡 in Arbeit. Tranche 0, Tranche 1 (§2.1–§2.6), Tranche 2 (§3.1–§3.4 inkl. §3.4c-Closeout), **Tranche 3 (§4.1–§4.7)**, **Tranche 4 (§5 H1–H6)** und **Tranche 5 (§6.1–§6.5)** vollständig abgeschlossen; Roadmap Schritte 31, 32, 33 und 34 sind auf ✅. Offen: Tranchen 6–8. Tranche 6 (Aggregat-Metriken / Drop-/Invalid-/Rate-Limit-Sichtbarkeit, RAK-33/34) ist der nächste Schritt.
...
- [x] Smoke-Test mit konkretem Suchwert: `scripts/smoke-tempo.sh` (State `tempo`) liest `correlation_id` aus dem API-Read-Pfad (`GET /api/stream-sessions/{id}`) und validiert exakt diesen Wert in Tempo via `GET /api/search?tags=mtrace.session.correlation_id=<UUID>`. "Trace sichtbar" ohne benannten Suchwert reicht nicht und passiert auch nicht — der Smoke failed bei `traces.length == 0`.
- [x] README v0.4.0-Sektion ergänzt um `make dev-tempo`-Hinweis und RAK-Bezug; `docs/user/local-development.md` §2.5 (neu) trennt Tempo (optional, Debug-Tiefe) von der eingebauten Session-Timeline (RAK-32). Beide Dokumente zeigen Tempo-Search-Beispiel mit `mtrace.session.correlation_id`-Tag.
