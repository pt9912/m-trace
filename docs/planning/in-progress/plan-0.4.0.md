...
- [ ] HTTP-Read-Handler lösen Project-Kontext vor Session-List, Session-Detail, Event-Read und Analyzer-Linking auf und reichen `project_id` in die Application-/Repository-Pfade weiter; Session-List- und Event-Cursor wechseln auf `cursor_version: 3` und enthalten den Project-Scope (`project_id` oder einen daraus abgeleiteten Scope-Hash). v2-Cursor ohne Project-Scope werden nach Aktivierung der projekt-skopierten Read-Pfade als `cursor_invalid_legacy` abgewiesen; v3-Cursor mit fremdem Scope liefern `cursor_invalid_malformed` und werden nicht für Project B akzeptiert.
...
- [ ] URL-Redaction ist vor Persistenz und Dashboard-Anzeige umgesetzt und getestet: keine Query-/Fragment-Speicherung, kein `userinfo`, keine signierten Query-Parameter, keine Credential-/Token-Parameter, keine tokenartigen Rohpfadanteile, keine URL-Labels in Prometheus. Der Event-Store enthält nur redigierte URL-Repräsentanten gemäß Abnahmegrenze und keinen stabilen Token-Hash. Tests injizieren auch Legacy-/Angreifer-Payloads mit rohen URLs in `meta.url`, `meta.network.url`, `meta.segment_url`, `meta.manifest_url` und unbekannten URL-artigen Meta-Keys und beweisen, dass nichts roh persistiert wird.
...
- [ ] Analyzer-Ergebnisse aus `POST /api/analyze` werden ohne explizite Session-/`correlation_id`-Bindung nicht in die Player-Timeline gemischt; mit expliziter Bindung ist die Verknüpfung über `(project_id, correlation_id)` testbar und ein fehlender/ungültiger Project-Kontext liefert den API-Kontraktfehler statt `detached`. Tests decken die Response-Hülle `{analysis, session_link}` für ungebundenen Request ohne Link-Felder (`detached`), nur bekannte `correlation_id` (`linked`), nur unbekannte `correlation_id` (`not_found_detached`), nur vorhandenen `session_id`-Fallback (`linked`), unbekannte `session_id` (`not_found_detached`), beide konsistent (`linked`), beide gesetzt mit unbekannter `correlation_id` und bekannter `session_id` (`not_found_detached`), beide im selben Project bekannt aber widersprüchlich (`conflict_detached`), Cross-Project-`correlation_id` mit/ohne passende `session_id` im Request-Project (`not_found_detached` bzw. `conflict_detached` nach Lookup-Regel), fehlenden/ungültigen Project-Kontext bei gesetzten Link-Feldern (`401`/Kontraktfehler) und Cross-Project-Mismatch ab. Alle Fälle sind dokumentiert.
- [ ] Der Breaking Change des `{analysis, session_link}`-Wrappers ist in Nutzer-/Smoke-Doku und Tests nachgezogen: `docs/user/stream-analyzer.md`, lokale Entwicklungs-/Smoke-Beispiele, API-Handler-Tests, Dashboard-/Client-API-Tests und analyzer contract fixtures erwarten ab Tranche 3 nicht mehr das flache `AnalysisResult` als direkte `POST /api/analyze`-Antwort.
