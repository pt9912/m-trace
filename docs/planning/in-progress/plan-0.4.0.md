...
- **Storage-/Repository-Prerequisite:** Der aktuelle `stream_sessions`-Store ist vor Tranche 3 `session_id`-global (`primary_key: [session_id]`, Repository-Reads per `Get(ctx, session_id)`). Vor Umsetzung der `project_id + session_id`-Abnahme muss Tranche 3 entweder Schema/Repository auf projekt-skopierte Session-Lookups heben (z. B. eindeutiger Key `(project_id, session_id)` und Resolver-Methoden mit `project_id`) oder verbindlich beweisen, dass SDK-`session_id`s projektübergreifend global eindeutig sind. In beiden Fällen ist ein Cross-Project-Kollisionstest Pflicht: gleiche `session_id` in zwei Projekten darf keine gemeinsame `correlation_id` erzeugen.
- **Analyzer-Linkage-Priorität:** Für explizite Analyzer-Verknüpfung gilt: `correlation_id` gewinnt vor `session_id`. Wenn beide angegeben sind, muss `session_id` zur Session mit dieser `correlation_id` auflösen; bei Mismatch antwortet die API weiter mit `200 OK` und unverändertem `AnalysisResult`, aber mit einem maschinenlesbaren Linkage-Status `session_link.status="conflict_detached"` in der API-/Dashboard-Hülle; das Ergebnis wird nicht in die Player-Timeline gemischt. Nur `session_id` ist als Fallback für Backfill-/Legacy-Flows erlaubt und muss serverseitig auf eine bestehende oder neu selbst-geheilte `correlation_id` aufgelöst werden.
...
- **URL-Datenschutz:** Segment-/Manifest-URLs dürfen weder Prometheus-Labels noch rohe, credential-haltige Persistenzwerte werden. Vor Persistenz/Anzeige gilt die feste Redaction-Matrix: Scheme, Host und nicht-sensitive Pfadsegmente dürfen erhalten bleiben; Query und Fragment werden vollständig entfernt; `userinfo` wird entfernt; signierte/credential-artige Query-Parameter (`token`, `signature`, `sig`, `expires`, `key`, `policy` und case-insensitive Varianten) werden nicht gespeichert; ein Pfadsegment ist tokenartig, wenn es ≥ 24 Zeichen lang ist und überwiegend Base64URL-/Hex-Zeichen enthält oder bekannte JWT-/SAS-/Signed-URL-Muster trägt. Tokenartige Pfadsegmente werden durch `:redacted:<sha256-prefix>` ersetzt, wobei der Hash stabil über denselben Eingabewert sein muss und kein Secret-Salt im Repo dokumentiert wird. Tests decken Query-/Fragment-Redaction, `userinfo`, signierte Query-Parameter, Token-Parameter, JWT-/Base64URL-Pfadsegmente und Stabilität des Hash-Präfixes ab.
...
- [ ] URL-Redaction ist vor Persistenz und Dashboard-Anzeige umgesetzt und getestet: keine Query-/Fragment-Speicherung, kein `userinfo`, keine signierten Query-Parameter, keine Credential-/Token-Parameter, keine tokenartigen Rohpfadanteile, keine URL-Labels in Prometheus. Der Event-Store enthält nur redigierte oder gehashte URL-Repräsentanten gemäß Abnahmegrenze.
...
- [ ] Analyzer-Ergebnisse aus `POST /api/analyze` werden ohne explizite Session-/`correlation_id`-Bindung nicht in die Player-Timeline gemischt; mit expliziter Bindung ist die Verknüpfung über `correlation_id` testbar. Tests decken nur `correlation_id`, nur `session_id`-Fallback, beide konsistent und beide im Konflikt ab. Alle Fälle sind dokumentiert.
