...
- [ ] `spec/backend-api-contract.md` §4 ist auf eine endpoint-spezifische Auth-Matrix umgestellt: `POST /api/playback-events` und Session-Reads sind tokenpflichtig; `POST /api/analyze` ist nur bei gesetzter `correlation_id` oder `session_id` tokenpflichtig, während ungebundene Analyze-Requests ohne Token erfolgreich `session_link.status="detached"` liefern. Tests pinnen alle vier Matrix-Zeilen.
