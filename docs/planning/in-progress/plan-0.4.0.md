...
- [ ] Tests decken die Degradationsmatrix ab: fehlende SDK-Felder, blockierte Browser-Timings, CORS-/Resource-Timing-Lücken als `network_detail_unavailable`-Events sowie mindestens ein Native-/Browser-Limitierungsfall ohne Netzwerksignal als sichtbares `network_signal_absent` werden als akzeptierte Degradation geprüft.
- [ ] Analyzer-Ergebnisse aus `POST /api/analyze` werden ohne explizite Session-/`correlation_id`-Bindung nicht in die Player-Timeline gemischt; mit expliziter Bindung ist die Verknüpfung über `correlation_id` testbar. Tests decken die Response-Hülle `{analysis, session_link}` für nur `correlation_id`, nur vorhandenen `session_id`-Fallback, unbekannte `session_id` (`not_found_detached`), beide konsistent und beide im Konflikt (`conflict_detached`) ab. Alle Fälle sind dokumentiert.
