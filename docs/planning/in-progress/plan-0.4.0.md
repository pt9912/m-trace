...
- Bekannter Folge-Punkt für 0.5.0: `SessionsService.ListSessions` lädt `network_signal_absent[]` heute pro Page-Eintrag einzeln (`ListBoundariesForSession` N+1, max. 1000 Sessions × Hard-Cap). Der Schreibpfad und der Detail-Read sind nicht betroffen. Bulk-Read-Port (`ListBoundariesForSessions(ctx, projectID, sessionIDs) (map[string][]SessionBoundary, error)`) kann ohne Vertragsbruch nachgereicht werden, sobald 0.5.0 Performance-Profiling aufnimmt.
