...
| Triggerfreie Follow-ups `RAK-102` / `RAK-103`           | offen (operator- / stakeholdergetrieben)    | Externe `apps/analyzer-api` (`RAK-102`) und `apps/control-plane` (`RAK-103`) bleiben in `open/plan-0.19.0.md` als Decision-Track; Abschluss ist dort als Decision-Record mit klaren Proceed-/Defer-Triggern in `docs/planning/open/plan-0.19.0.md` verpflichtend. |
