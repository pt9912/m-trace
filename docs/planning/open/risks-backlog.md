...
| R-8 | Der Observability-Smoke prüft Forbidden-Labels für `mtrace_*`-Metriken heute mit Namen und generischen Suffixen (`_url`, `_uri`, `_token`, `_secret`). Neue Credential-/URL-Label-Namensformen können trotzdem durchrutschen, wenn sie nicht unter diese Muster fallen. | `plan-0.4.0.md` §7.4 (Review F-1) | Tranche 7 | ⬜ | Forbidden-Pattern zentralisieren und bei Tranche 7 in die normative Cardinality-Doku übernehmen. Triggerschwelle: spätestens bei Tranche-7-Cardinality-Doku oder beim ersten neuen `mtrace_*`-Counter mit Labels; dann Pattern-Set und Smoke gemeinsam prüfen. |
