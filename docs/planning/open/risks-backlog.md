...
> **Stand**: 2026-05-06 (0.7.0 Tranche 4 — neues R-12 für WebRTC-`getStats()`-Schema-Drift-Review-Gate; R-1..R-11 unverändert)  
...
| R-12 | WebRTC-`getStats()`-Schema-Drift zwischen Browser-Major-Versionen (Chromium/Firefox/Safari) kann die in `spec/telemetry-model.md` §3.5 spezifizierte WebRTC-Aggregat-Allowlist (`connection_state`, `ice_state`, `dtls_state`) und das `getStats()`-Subset (Muss-/Soll-Felder pro `RTCStatsType`-Gruppe) ungültig machen. Auswirkung im `0.7.0`-Scope **null**, weil weder produktiver `mtrace_webrtc_*`-Counter noch Player-SDK-WebRTC-Adapter existieren — `make smoke-webrtc-prep` ist endpoint-only und vom Drift nicht betroffen. Bei einer Folge-Phase mit produktiver WebRTC-Telemetrie wird der Drift release-blockierend. | `plan-0.7.0.md` §5 Tranche 4 (RAK-49); `spec/telemetry-model.md` §3.5 Future-Telemetry-Notiz | bei Triggerschwelle | ⬜ | **Spec-/Adapter-Review-Gate**: bei Browser-Major-Version mit `getStats()`-Schema-Änderung wird die WebRTC-Allowlist (§3.2) plus die Future-Telemetry-Notiz (§3.5) reviewed; konkrete Smoke-/Contract-Test-Updates sind erst dann release-blockierend, wenn ein produktiver WebRTC-Telemetrie-Pfad existiert (RAK-51 / Folgeplan zu `plan-0.7.0`). Triggerschwelle: Chromium/Firefox/Safari Major-Bump ändert `RTCStatsType`-Felder oder Enum-Werte für `connectionState`/`iceConnectionState`/`dtlsState`, **oder** `0.7.0`-Folgeplan beginnt eine produktive `mtrace_webrtc_*`-Implementierung. |
