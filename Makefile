...
# `make smoke-load-multi-tenant` (R-26) fährt den Noisy-Neighbor-Fall
# gegen N geseedete Lab-Projekte (MTRACE_LAB_PROJECTS) auf einer
# Instanz: lab-1 saturiert den Limiter, lab-2..N müssen isoliert
# bleiben (0x 429, p95 im Budget); der Noisy wird nachweislich
# gedrosselt. Default-Limiter aktiv. Destruktiv, opt-in.
smoke-load-multi-tenant:
	MODE=multi-tenant bash scripts/smoke-load.sh
...
# `make smoke-scaleout-fairness` (R-26) — Fairness-Nachweis des shared
# Redis-Ingest-Limiters über 2 Replicas: throttled darf die 2. Replica
# das Per-Projekt-Budget NICHT verdoppeln (Gate <= 1.15x, invertiert
# den gemessenen per-Replica-2,01x-Befund), plus Noisy-Neighbor über
# den LB (Victims isoliert). Opt-in (NICHT in `make gates`).
smoke-scaleout-fairness:
	FAIRNESS=1 bash scripts/smoke-scaleout-load.sh
...
# `make smoke-cutover` — opt-in-Smoke für den Cutover (ADR-0007). Fährt gegen
# ein ephemeres Lab: `doctor` (Pre-Flight) grün, `profile` grün auf gesunder
# Quelle, und `profile` bricht auf einer korrupten Quelle ab (self-type-
# Tripwire). Ephemer (eigenes Netz + PG), opt-in (NICHT in `make gates`).
smoke-cutover:
...
# `make verify-closure-notes` ist der strukturelle Closure-Note-Gate
