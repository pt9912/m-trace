...
func TestHTTPStreamAnalyzer_AnalyzeManifest_TextWithBaseURL(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		receivedBody = string(body)
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"status":"ok","analyzerVersion":"0.5.0","analyzerKind":"hls","playlistType":"unknown","details":null,"findings":[]}`))
	}))
	defer server.Close()
...
func TestHTTPStreamAnalyzer_AnalyzeManifest_LimitsResponseSize(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"status":"ok","analyzerVersion":"0.5.0","analyzerKind":"hls","playlistType":"unknown","details":"`))
...
func TestHTTPStreamAnalyzer_AnalyzeManifest_ReturnsDomainErrorForStatusError(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"status":"error","analyzerVersion":"0.5.0","analyzerKind":"hls","code":"manifest_not_hls","message":"not HLS","details":{"firstLine":"<html>"}}`))
	}))
	defer server.Close()
...
func TestHTTPStreamAnalyzer_WithHTTPClient(t *testing.T) {
	...
	rt := roundTripperFunc(func(_ *http.Request) (*http.Response, error) {
		called++
		body := `{"status":"ok","analyzerVersion":"0.5.0","analyzerKind":"hls","playlistType":"unknown","details":null,"findings":[]}`
