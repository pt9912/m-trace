...
// spec/telemetry-model.md: http.method, http.route,
...
func newTestServerWithTracerProvider(t *testing.T, tp *sdktrace.TracerProvider) *httptest.Server {
	...
	router := apihttp.NewRouter(uc, sessionsService, nil, resolver, resolver, publisher.Handler(), nil, nil, nil, nil, nil, nil, nil, nil, nil, false, tracer, logger)
